...
export const demoViolinSmoothe = (newData: string, config: object): void => {
    ...
    const svg = d3.select("#violin-smoothe")
        .append("svg")
    ...
    d3.csv(newData, function (data) {
        ...
        const sumstat = d3.group()  // nest function allows to group the calculation per level of a factor
            .key(function (d) { return d.Species; })
            .rollup(function (d) {   // For each key..
                const input = d.map(function (g) { return g.Sepal_Length; })    // Keep the variable called Sepal_Length
